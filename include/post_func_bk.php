	...
	switch($reqPost)
	{
		...
		case 'mainCategory':
			...
			if(isset($_POST['submit']))
			{
				if($categoryName!='')
				{
					...
					if($category_info == "")
					{
						$array_val = array("parent_id" => "", "category_name" => $categoryName, "created_by" => $_SESSION['user_login'], "status" => 'Active', "created_date" => date('Y-m-d H:i:s'));
		...
		case 'addsubcategory':
			...
			if(isset($_POST['submit']))
			{
				if($maincategory!='' && $subcategory!='')
				{
					...
					if($category_info != "")
					{
						$array_val = array("parent_id" => $maincategory, "category_name" => $subcategory, "created_by" => $_SESSION['user_login'], "status" => 'Active', "created_date" => date('Y-m-d H:i:s'));
						$insert_info = insert_table_data($array_val, 'tbl_category');
						$last_id = last_id();
						if($last_id > 0)
						{
		...
		case 'QuickTest':
			...
			if(isset($_POST['submit']))
			{
				...
				if($exprity_date!='') 
				{
					...
					$get_questions = $functions->check_question_exists($categoryId, $typeId);
						...
						if($maxQuestion <= $totalQuestion)
						{
							if($categoryId!="" && $typeId!="")
							{
								...
								if($last_id > 0)
								{
									...
									$strQry = "SELECT id, question_type FROM tbl_questions WHERE id != '' AND category_id = '".$categoryId."' AND type_id = '".$typeId."' AND is_deleted = '0' AND is_block = '0' AND status = 'Active' ORDER BY RAND() LIMIT ".$maxQuestion." ";
									$result = mysqli_query($conn, $strQry);
									if(mysqli_num_rows($result) > 0)
									{
										while($row = mysqli_fetch_array($result))
										{
											if($row['question_type'] == "Live,Demo" || $row['question_type'] == "Live")
											{
												...
												if($questionInfo !='')
												{
													...
													{
														$array_val = array("test_id" => $randomNo, "question_id" => $row['id'], "answer" => "Pending", "user_answer" => "", "solve_time" => "");
					...
					{
						echo json_encode(array('code'=>0, 'message'=>'Questions not found against this category.'));
						exit;
					}				
				}
				else
							...
							if($categoryId!="" && $typeId!="")
							{
								...
								if($last_id > 0)
								{
									...
									if(mysqli_num_rows($result) > 0)
									{
										while($row = mysqli_fetch_array($result))
										{
											if($row['question_type'] == "Live,Demo" || $row['question_type'] == "Demo")
											{
												$questionInfo = get_table_data('tbl_questions', 'id="'.$row['id'].'" ');
												if($questionInfo !='')
												{
		...
		case 'GenerateTest':
			...
			if(isset($_POST['submit']))
			{
				...
				$group = '';
				$difficulty_level = '';
				if(isset($_REQUEST['test_mode'])){$test_mode = implode("','",$_REQUEST['test_mode']);}
