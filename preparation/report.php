<?php 
	...
	if($testid == ""){
		header("Location:previous_test_list");
	}

	$report_type_id = 0;
	$ttt = get_table_data('tbl_quick_test', 'random_id="'.$testid.'" ');
	if($ttt!='')
	{
		$report_type_id = $ttt[0]->type_id;
	}
						...
						<?php
							if($type_info!="")
							{
								foreach($type_info as $key => $val)
								{
									...
									if($report_type_id == $val->id){

									?>
									<div class="tab-pane active" id="<?php echo $val->id;?>" role="tabpanel">
